...
function distribuirCerticado(){
	...
	read -p "He de crear una copia para renombrarlo " importando
...
echo "¿Quieres distribuir a la RAiz el certificado?- (ca.req) s/n"
read opcion 
if [[ $opcion == "s" ]]
then
	read -p "IP Equipo RAIZ: " ipDestino
	read -p "Ruta completa Equipo destino:/home/$usuario/tmp/  " rutaFich
	mkdir /home/$usuario/tmp
	cp /home/$usuario/ca/easy-rsa/pki/reqs/ca.req /home/$usuario/tmp/subca.req 
	read -p "Usuario de IP destino: " usuarioDestino

	scp /home/$usuario/tmp/subca.req  $usuarioDestino@$ipDestino:$rutaFich
	read -p "He de crear una copia para renombrarlo " importando
